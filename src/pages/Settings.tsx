  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 space-y-6">
      ...
      <div className="space-y-8">
        ...
        {/* Server Config Tab */}
        {!isMaster && currentClinic?.allowServerConfig && (activeTab === 'all' || activeTab === 'server_config') && (
          <ServerConfigTab />
        )}
      </div>
...
function ServerConfigTab() {
  ...
  const [dbType, setDbType] = useState(currentClinic?.dbType || 'local_sql');
  const [dbHost, setDbHost] = useState(currentClinic?.dbHost || '');
  const [dbPort, setDbPort] = useState(currentClinic?.dbPort || '');
  const [dbUser, setDbUser] = useState(currentClinic?.dbUser || '');
  const [dbPass, setDbPass] = useState(currentClinic?.dbPass || '');
  const [dbName, setDbName] = useState(currentClinic?.dbName || '');

  const handleSave = () => {
    if (!currentClinic) return;
    
    // Only master or clinic admin can change
    if (currentUser?.role !== 'admin' && currentUser?.role !== 'master_admin') {
      alert("ليس لديك صلاحية لتعديل السيرفرات");
      return;
    }
  ...
  return (
    <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm mb-8">
      <div className="flex items-center gap-3 mb-6 border-b border-slate-100 pb-4">
        ...
        <div>
          <h2 className="text-lg font-bold text-slate-800">إعدادات السيرفر وقاعدة البيانات</h2>
          <p className="text-sm text-slate-500">قم بربط المنشأة بقاعدة بيانات مخصصة لفصل بياناتك عن السيرفر المشترك.</p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="md:col-span-2">
          <label className="block text-sm font-bold text-slate-700 mb-1.5">نوع قاعدة البيانات</label>
          <select
            value={dbType}
            onChange={e => setDbType(e.target.value)}
            className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold text-slate-800 focus:outline-none focus:border-indigo-500"
          >
            <option value="cloud_firebase">Cloud Firebase (NoSQL)</option>
            <option value="local_sql">Local SQL (SQLite/PostgreSQL)</option>
            <option value="external_sql">External SQL (Supabase/Neon)</option>
          </select>
        </div>
        ...
        {dbType !== 'cloud_firebase' && (
          <>
            <div>
              <label className="block text-sm font-bold text-slate-700 mb-1.5">مضيف السيرفر (Host)</label>
              <input
                type="text"
                value={dbHost}
                onChange={e => setDbHost(e.target.value)}
                placeholder="e.g. localhost, aws.rds.amazon.com"
                className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold text-slate-800 focus:outline-none focus:border-indigo-500"
              />
            </div>
            <div>
              <label className="block text-sm font-bold text-slate-700 mb-1.5">المنفذ (Port)</label>
              <input
                type="text"
                value={dbPort}
            ...
            <div>
              <label className="block text-sm font-bold text-slate-700 mb-1.5">اسم المستخدم (DB User)</label>
              <input
                type="text"
                value={dbUser}
            ...
            <div>
              <label className="block text-sm font-bold text-slate-700 mb-1.5">كلمة المرور (DB Password)</label>
              <input
                type="password"
                value={dbPass}
            ...
            <div className="md:col-span-2">
              <label className="block text-sm font-bold text-slate-700 mb-1.5">اسم قاعدة البيانات (DB Name)</label>
              <input
                type="text"
                value={dbName}
      ...
      <div className="flex justify-end mt-6">
        <button
          onClick={handleSave}
          className="px-6 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-sm font-bold transition-colors"
        >
          حفظ إعدادات السيرفر
        </button>
      </div>
