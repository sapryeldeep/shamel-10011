    ...
    <div className="h-full flex flex-col gap-6">
      ...
      {activeTab === 'new' && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 overflow-y-auto custom-scrollbar flex-1 print:fixed print:inset-0 print:z-[9999] print:m-0 print:w-full print:h-full print:border-0 print:shadow-none print:rounded-none">
          ...
          {/* Quick Pre-configured Report Templates */}
          <div className="mb-6 bg-amber-50/50 border border-amber-200/60 p-4 rounded-xl print:hidden">
            <div className="flex items-center gap-2 mb-3">
              <Sparkles size={18} className="text-amber-600" />
              <h6 className="font-bold text-amber-900 text-sm">نماذج التقارير السريرية الجاهزة ({getSpecialtyName(specialtyKey)})</h6>
            </div>
            <select
              value=""
              onChange={(e) => {
                const availableTemplates = state.customReportTemplatesStore?.[specialtyKey] || getReportTemplatesForSpecialty(specialtyKey);
                const tmpl = availableTemplates.find(t => t.id === e.target.value);
                if (tmpl) {
                  if (tmpl.chiefComplaint) setChiefComplaint(tmpl.chiefComplaint);
                  if (tmpl.medicalHistory) setMedicalHistory(tmpl.medicalHistory);
                  if (tmpl.physicalExamination) setPhysicalExamination(tmpl.physicalExamination);
                  if (tmpl.diagnosis) setDiagnosis(tmpl.diagnosis);
                  if (tmpl.treatmentPlan) setTreatmentPlan(tmpl.treatmentPlan);
                }
              }}
              className="w-full md:w-1/2 px-4 py-2.5 bg-white border border-amber-200 rounded-lg text-sm text-slate-800 font-bold focus:outline-none focus:border-amber-500 shadow-sm transition-colors"
            >
              <option value="">-- اختر نموذج تقرير جاهز لتحميله --</option>
              {(state.customReportTemplatesStore?.[specialtyKey] || getReportTemplatesForSpecialty(specialtyKey)).map(t => (
            ...
            <div className="mb-6 bg-blue-50/50 p-4 rounded-xl border border-blue-200">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                {Object.entries(selectedReport.specialtyFields).map(([k, val]) => {
                  if (!val) return null;
                  const fields = state.customSpecialtyFieldsStore?.[selectedReport.specialtyKey || ''] || getSpecialtyFields(selectedReport.specialtyKey || '');
