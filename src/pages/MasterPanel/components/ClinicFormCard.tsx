    ...
    <div className="bg-slate-50/80 border border-slate-200 rounded-2xl p-6 mb-8 shadow-sm">
      ...
      <div className="bg-blue-50/50 border border-blue-200 p-4 rounded-xl mb-6">
        ...
        {dbType === 'cloud_firebase' && (
          <div className="bg-white p-4 rounded-xl border border-blue-200 grid grid-cols-1 gap-3 animate-fade-in bg-blue-50/10">
            <div>
              <label className="block text-[10px] font-bold text-slate-500 mb-1 text-right">رابط قاعدة بيانات فايربيز للعيادة (Firebase RTDB URL)</label>
              <input
                type="text"
                value={dbHost}
                onChange={e => setDbHost(e.target.value)}
                placeholder="https://my-clinic-db.firebaseio.com"
                className="w-full px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-xs font-mono"
                dir="ltr"
              />
              <p className="text-[10px] text-slate-400 mt-1">اترك الحقل فارغاً لاستخدام قاعدة بيانات السحابة الافتراضية للنظام.</p>
            </div>
          </div>
        )}
