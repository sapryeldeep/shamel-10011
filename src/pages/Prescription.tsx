...
import { 
  ...
  Search, UserCheck, History, User, Edit3
} from 'lucide-react';
...
export default function Prescription() {
  ...
  const [editingRxPatient, setEditingRxPatient] = useState<string | null>(null);
  ...
  const currentSpecialtyName = settingsForm.specialty || currentClinic?.specialty || '';
  const specialtyKey = currentSpecialtyName ? (MEDICAL_SPECIALTIES.find(s => s.name === currentSpecialtyName || s.id === currentSpecialtyName)?.id || 'general_hospital') : 'general_hospital';
  const currentSpecialtyFields = state.customSpecialtyFieldsStore?.[specialtyKey] || getSpecialtyFields(specialtyKey);
  ...
  const printRx = () => {
    ...
    if (rxPatient) {
      ...
      if (editingRxId) {
        // Handle edit existing prescription
        const updatedRxList = currentPatientRx.map(rx => {
          if (rx.id === editingRxId || (rx.date === editingRxId)) {
            return {
              ...rx,
              ...
              specialtyNeeds: rxSelectedNeeds,
              clinicId: rx.clinicId || currentClinicId
            };
          }
          return rx;
        });
        ...
        updateState({
          rxStore: {
            ...(state.rxStore || {}),
            [rxPatient]: updatedRxList
          }
        });
        
        logAction(
          'تعديل روشتة',
          `تم تعديل الروشتة العلاجية المحفوظة مسبقاً للمريض «${rxPatient}» بتشخيص «${rxDiag || 'كشف دوري'}».`,
          'medical',
          { operationType: 'update', targetName: rxPatient }
        );
      } else {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-xs flex flex-wrap items-center justify-between gap-4 print:hidden">
        <div className="flex items-center gap-3">
          ...
          >
            <History size={16} /> سجل الروشتات السابقة
          </button>
          ...
          {activeTab === 'create' ? (
            ...
            <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm flex flex-col h-full overflow-hidden">
              ...
              <div className="space-y-4 flex-1 overflow-y-auto custom-scrollbar pr-1">
                ...
                {/* Specialty-Specific Clinical Examination Fields */}
                {currentSpecialtyFields.length > 0 && (
                  <div className="bg-blue-50/50 p-3.5 rounded-xl border border-blue-200 shadow-2xs space-y-3">
                    <h6 className="text-xs font-bold text-blue-800 flex items-center gap-1.5 border-b border-blue-100 pb-2">
                      <Stethoscope size={14} /> الكشف السريري المتخصص ({currentSpecialtyName || 'العيادة'})
                    </h6>
                        ...
                        <div key={field.key} className={field.type === 'textarea' ? 'md:col-span-2' : ''}>
                          ...
                          {field.type === 'textarea' ? (
                            <textarea
                              ...
                              onChange={e => setSpecialtyData({ ...specialtyData, [field.key]: e.target.value })}
                ...
                {/* Custom Drug Input */}
                {(isDeveloper || currentClinic?.allowPrescriptionExternal !== false) && (
                ...
                <div className="bg-emerald-50/70 p-3.5 rounded-xl border border-emerald-200 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-emerald-900 flex items-center gap-1.5">
                      ...
                      احتياجات ومستلزمات المريض الخاصة بالتخصص ({getSpecialtyName(specialtyKey)}):
                    </span>
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar pr-1 space-y-3">
                {(() => {
                  const allRx = Object.entries(state.rxStore || {}).flatMap(([pName, rxArr]) => {
                    return (rxArr as any[]).map(rx => ({ ...rx, patientName: rx.patientName || pName }));
                  })
                  .filter(rx => currentClinicId === 'master' || rx.clinicId === currentClinicId || !rx.clinicId) // Fallback for old records
        ...
        <div className="xl:col-span-7">
          ...
          >
            ...
            <div>
              ...
              {/* Specialty Clinical Findings */}
              {Object.keys(specialtyData).length > 0 && Object.values(specialtyData).some(v => String(v).trim() !== '') && (
                <div className="mb-5 border border-slate-200 rounded-xl overflow-hidden text-xs">
                  <div className="bg-slate-100 text-slate-700 font-bold px-3 py-1.5 border-b border-slate-200">
                    مؤشرات الكشف السريري
                  </div>
                  <div className="p-3 grid grid-cols-1 md:grid-cols-2 gap-3 bg-slate-50/50">
                    {Object.entries(specialtyData).filter(([_, val]) => String(val).trim() !== '').map(([key, val]) => {
                      const fieldLabel = currentSpecialtyFields.find(f => f.key === key)?.label || key;
                      return (
                        <div key={key}>
                          <span className="block text-[10px] text-slate-500 font-bold">{fieldLabel}</span>
                          <strong className="text-slate-800 text-sm whitespace-pre-line leading-relaxed">{val}</strong>
                        </div>
                      );
                    })}
                  </div>
                ...
                {rxSelectedNeeds.length > 0 && (
                  <div className="mt-4 p-3 bg-emerald-50/80 border border-emerald-200 rounded-xl text-xs text-emerald-950">
                    <strong className="block text-emerald-900 mb-1 border-b border-emerald-200/60 pb-1">
                      المستلزمات واحتياجات المريض الموصى بها ({getSpecialtyName(specialtyKey)}):
                    </strong>
