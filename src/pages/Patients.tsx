  ...
  const getCombinedVisitsForActivePatient = () => {
    ...
    statusRecords.forEach(r => {
      combined.push({
        ...
        specialtyFields: r.specialtyFields,
                    ...
                    {last5Visits.length === 0 ? (
                      ...
                      last5Visits.map((v, idx) => (
                        ...
                        >
                          ...
                          {/* Custom Specialty Fields */}
                          {v.specialtyFields && Object.keys(v.specialtyFields).length > 0 && (
                            <div className="bg-indigo-50/50 border border-indigo-100 p-3 rounded-xl mt-2">
                              <h5 className="text-[11px] font-bold text-indigo-800 mb-2 flex items-center gap-1.5"><Stethoscope size={14}/> الفحص السريري الدقيق ({getSpecialtyName((state.clinics.find(c => String(c.id) === String(getClinicId()))?.specialty) || 'general_hospital')})</h5>
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                                {(() => {
                                  const currentClinic = state.clinics.find(c => String(c.id) === String(getClinicId()));
                                  const specialtyKey = currentClinic?.specialty ? (MEDICAL_SPECIALTIES.find(s => s.name === currentClinic.specialty || s.id === currentClinic.specialty)?.id || 'general_hospital') : 'general_hospital';
                                  const fields = state.customSpecialtyFieldsStore?.[specialtyKey] || getSpecialtyFields(specialtyKey);
                                  
                                  return Object.entries(v.specialtyFields).map(([k, val]) => {
                                    if (!val) return null;
                                    const fieldConfig = fields.find(f => f.key === k);
                                    const label = fieldConfig ? fieldConfig.label : k;
                                    return (
                                      <div key={k} className="bg-white px-2 py-1.5 rounded border border-indigo-100 shadow-sm flex flex-col">
                                        <span className="text-indigo-500 text-[10px] font-bold mb-0.5">{label}</span>
                                        <span className="text-indigo-950 font-bold">{val}</span>
                                      </div>
                                    );
                                  });
                                })()}
            ...
            <div>
              ...
              <div className="grid grid-cols-4 gap-2">
                <input
                  ...
                  value={bp}
                  onChange={e => setBp(e.target.value)}
                  className="px-2 py-1.5 bg-slate-50 border border-slate-200 rounded text-[11px] text-center focus:ring-2 focus:ring-blue-500"
                />
                <input
                  type="text"
                  placeholder="HR (72)"
              ...
              return (
                <div className="border-t border-slate-200 pt-4 mt-4">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h6 className="font-bold text-slate-800 text-xs">متطلبات سريرية مخصصة ({getSpecialtyName(specialtyKey)})</h6>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {fields.map(field => (
                      <div key={field.key} className={field.key.includes('Notes') || field.key.includes('Exam') ? 'sm:col-span-2' : ''}>
                        <label className="block text-[10px] font-bold text-slate-500 mb-1">{field.label}</label>
                        <input
                          type="text"
                          value={specialtyData[field.key] || ''}
                          onChange={e => setSpecialtyData(prev => ({ ...prev, [field.key]: e.target.value }))}
