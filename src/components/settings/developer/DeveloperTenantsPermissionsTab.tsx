...
export default function DeveloperTenantsPermissionsTab({
  ...
  const [allowPrescriptionEdit, setAllowPrescriptionEdit] = useState(true);
  const [allowPrescriptionExternal, setAllowPrescriptionExternal] = useState(true);
  ...
  const [allowServerConfig, setAllowServerConfig] = useState(false);
  ...
  const handleEditClick = (c: Clinic) => {
    ...
    setAllowPrescriptionEdit(c.allowPrescriptionEdit !== false);
    setAllowPrescriptionExternal(c.allowPrescriptionExternal !== false);
    ...
    setAllowServerConfig(c.allowServerConfig || false);
      ...
      const updatedClinics = clinics.map(c => {
        if (c.id === editingClinicId) {
          return {
            ...
            allowPrescriptionEdit,
            allowPrescriptionExternal,
        ...
        <form onSubmit={handleSaveClinic} className="space-y-6">
          ...
          <div className="space-y-3 bg-slate-50 p-4 rounded-2xl border border-slate-200">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              {/* Prescription Edit */}
              <div className="flex items-center justify-between p-2.5 bg-white rounded-xl border border-slate-200">
                <span className="text-xs font-bold text-slate-700">تعديل الروشتة بعد الحفظ والطباعة</span>
                <button
                  type="button"
                  onClick={() => setAllowPrescriptionEdit(!allowPrescriptionEdit)}
                  className={`w-11 h-6 rounded-full transition-colors relative cursor-pointer ${
                    allowPrescriptionEdit ? 'bg-indigo-600' : 'bg-slate-300'
                  }`}
                >
                  <span className={`block w-4 h-4 rounded-full bg-white transition-transform absolute top-1 ${
                    allowPrescriptionEdit ? 'left-6' : 'left-1'
                  }`} />
                </button>
              </div>

              {/* External Prescription Drugs */}
              <div className="flex items-center justify-between p-2.5 bg-white rounded-xl border border-slate-200">
                <span className="text-xs font-bold text-slate-700">إضافة أدوية غير مدرجة بالمخزون للروشتة</span>
                <button
                  type="button"
                  onClick={() => setAllowPrescriptionExternal(!allowPrescriptionExternal)}
                  className={`w-11 h-6 rounded-full transition-colors relative cursor-pointer ${
                    allowPrescriptionExternal ? 'bg-indigo-600' : 'bg-slate-300'
                  }`}
                >
                  <span className={`block w-4 h-4 rounded-full bg-white transition-transform absolute top-1 ${
                    allowPrescriptionExternal ? 'left-6' : 'left-1'
                  }`} />
                </button>
              </div>
              ...
              <div className="flex items-center justify-between p-3 bg-indigo-50 border border-indigo-200 rounded-xl md:col-span-2 mt-2">
                <div>
                  <h6 className="text-sm font-bold text-indigo-900">إدارة قواعد البيانات وسيرفرات التخزين (صلاحية العيادة)</h6>
                  <p className="text-xs text-indigo-700 mt-1">السماح لمدير المنشأة بربط سيرفر أو قاعدة بيانات خارجية خاصة به وفصل بياناته عن السيرفر المشترك.</p>
                </div>
                <button
                  type="button"
                  onClick={() => setAllowServerConfig(!allowServerConfig)}
                  className={`w-11 h-6 rounded-full transition-colors relative cursor-pointer ${
                    allowServerConfig ? 'bg-indigo-600' : 'bg-slate-300'
                  }`}
                >
                  <span className={`block w-4 h-4 rounded-full bg-white transition-transform absolute top-1 ${
                    allowServerConfig ? 'left-6' : 'left-1'
                  }`} />
                </button>
              </div>

              {/* Prescription Edit After Saving */}
              <div className="flex items-center justify-between p-2.5 bg-white rounded-xl border border-slate-200">
                <span className="text-xs font-bold text-slate-700">تعديل الروشتة بعد الحفظ والإصدار</span>
                <button
                  type="button"
                  onClick={() => setAllowPrescriptionEdit(!allowPrescriptionEdit)}
                  className={`w-11 h-6 rounded-full transition-colors relative cursor-pointer ${
                    allowPrescriptionEdit ? 'bg-emerald-600' : 'bg-slate-300'
                  }`}
                >
                  <span className={`block w-4 h-4 rounded-full bg-white transition-transform absolute top-1 ${
                    allowPrescriptionEdit ? 'left-6' : 'left-1'
