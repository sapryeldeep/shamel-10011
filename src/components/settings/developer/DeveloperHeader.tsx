...
import { 
  ...
  Database, Lock, FileSpreadsheet, FileText, Server, BookOpen, Rocket, Stethoscope
} from 'lucide-react';
import * as XLSX from 'xlsx';
import { Clinic } from '../../../types';
import { getFormattedDateTime } from '../../../lib/utils';

export type DeveloperTabType = 'dashboard' | 'tenants_permissions' | 'accounts_preview' | 'system_updates' | 'system_cloud' | 'maintenance_health' | 'clinical_config';
...
export default function DeveloperHeader({
  ...
  return (
    <div className="bg-slate-900 text-white rounded-3xl p-6 shadow-xl border border-slate-800 space-y-6">
      ...
      {/* Main Modern Navigation Tabs - Responsive Grid for Tablets (iPad, Tablet Portrait/Landscape) */}
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-7 gap-2 p-2 bg-slate-950 rounded-2xl border border-slate-800">
        <button
        ...
        >
          ...
          <span className="truncate">7. متطلبات سريرية</span>
        </button>
      </div>
    </div>
  );
